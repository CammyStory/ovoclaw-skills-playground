...
// the agent can re-arm it. Generous vs the ~1–5 min tick interval to avoid false
// alarms.
const AUTO_STALL_AFTER_MS = 15 * 60_000

type AutoHealth = 'off' | 'starting' | 'healthy' | 'stalled'

function autoReplyHealth(ar: AutoReplyState): { state: AutoHealth; last_tick_at: string | null; stale_minutes: number | null; recommendation?: string } {
  if (ar.status !== 'running') return { state: 'off', last_tick_at: ar.lastCheckedAt ?? null, stale_minutes: null }
  const now = Date.now()
  const ref = ar.lastCheckedAt ?? ar.startedAt
  if (!ref) return { state: 'starting', last_tick_at: null, stale_minutes: null }
  ...
  if (age < AUTO_STALL_AFTER_MS) return { state: 'healthy', last_tick_at: ar.lastCheckedAt, stale_minutes: Math.round(age / 60000) }
...
function autoReplyView(ar: AutoReplyState) {
  return {
    auto_reply: {
      status: ar.status,
...
async function cmdAutoReplyStart() {
  ...
  const ar: AutoReplyState = { status: 'running', startedAt: new Date().toISOString(), repliesSent: 0 }
  await saveAutoReply(ar)
  ok({
    status: 'auto_reply_started',
    ...autoReplyView(ar),
    next_step:
      'Auto-reply is ON. The flag alone does NOT answer messages — now set up a recurring SCHEDULED TASK with your platform scheduler (a QClaw scheduled task / cron / routine) that EACH run: checks `auto-reply-status` → if running, runs `check-inbox` and `respond` to each un-replied thread → then exits silently (never auto-approve new connection requests). See "After login: serve the agent" in SKILL.md.',
  })
}

async function cmdAutoReplyStop() {
  const ar = await loadAutoReply()
  const next: AutoReplyState = { ...ar, status: 'off' }
