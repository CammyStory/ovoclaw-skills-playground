...
async function cmdAcceptPending(flags) {
    ...
    if (!isConfirmed(flags)) {
        ...
        needsConfirmation('approve', { request_id: requestId, will: 'Admit this requester — they can then exchange messages with your agent.' }, "Approve this connection request so they can talk to me? (Run `requests` first if you want to review who's asking.)", `approve --request-id ${requestId} --confirmed`);
    }
